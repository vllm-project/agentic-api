...
including Responses, Messages, and Interactions, and to orchestrate server-side
tool execution for tool calls generated by vLLM core.
...
- Preserve unknown tool and item shapes without executing them by default.
- Support model aliasing and routing needed for local vLLM core-backed Codex
  usage.
...
When vLLM core produces a tool call that is owned by the gateway, vLLM Agentic API
should dispatch it, capture the result, append the tool output, and continue the
agentic loop. The tool system should make ownership explicit so client-owned
tools can be returned to the caller while gateway-owned tools execute inside the
server.
...
The fourth major API goal is Interactions.

Interactions should provide the higher-level agentic workflow surface once the
Responses and Messages foundations are solid. This work should reuse the same
state, orchestration, and tool execution model rather than creating a parallel
stack.

Core work:

- Define the Interactions API contract.
- Reuse the shared execution loop for model calls and tool calls.
- Support durable interaction state.
- Identify which interaction-level concepts belong in this repository and which
  belong in clients or gateways.

### 6. Broader Stateful Agentic APIs

After Responses, Codex support, Messages, and Interactions, the project should
add the remaining stateful APIs needed by core agentic applications.
