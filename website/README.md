...
The effects stylesheet adds blue/gold lighting behind hero sections, brief entrance and diagram signal animations, and matching hover/focus highlights on cards. Automatic motion finishes within four seconds; hover interactions replay one short diagram cycle. Motion uses CSS only, adds no dependencies, and respects `prefers-reduced-motion`. Content stays visible without animation support.
