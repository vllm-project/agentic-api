...
Design intent:
- Production code should use the shared DB engine plumbing from `vllm_responses.db` (instrumentation,
...
class DBResponseStore:
    ...
    def from_db_url(cls, *, db_url: str) -> Self:
        ...
        Note: this bypasses `vllm_responses.db` engine plumbing (instrumentation and SQLite PRAGMAs).
