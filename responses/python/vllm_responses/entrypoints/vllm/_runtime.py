...
def _is_vllm_responses_endpoint(route: BaseRoute, endpoint: Callable[..., Any]) -> bool:
...
def _assert_responses_route_ownership(app: FastAPI) -> None:
    ...
    if len(post_routes) != 1 or not _is_vllm_responses_endpoint(
    ...
    if len(get_routes) != 1 or not _is_vllm_responses_endpoint(
...
def _cleanup_responses_family_routes(app: FastAPI) -> None:
    ...
    for route in app.router.routes:
        ...
        if not matched:
            ...
            continue
        if _is_vllm_responses_endpoint(
            route, serving.create_model_response
        ) or _is_vllm_responses_endpoint(route, serving.retrieve_model_response):
            cleaned_routes.append(route)
